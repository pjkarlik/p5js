...
/*
 * Cube Render - draws a grid for each side and applies the simplex noise
 */
    ...
    for (var k = 0; k < spacing; k++) {
      for (var j = 0; j < spacing; j++) {
        for (var i = 0; i < spacing; i++) {
          ...
          p.push();
          if (i === 0 || i === spacing - 1|| j === 0 || j === spacing - 1|| k === 0 || k === spacing - 1) {
          ...
          if (i === 0 || i === spacing - 1|| j === 0 || j === spacing - 1|| k === 0 || k === spacing - 1) {
            var nPoint = p.abs(
              generator.simplex3(iteration * i,
                iteration * j, (iteration * k) + timeStop)
              ) * strength;
  ...
  p.shader = function(noise, i, j){
    ...
    switch(shaderType) {
      ...
      case 'larvel':
        ...
        r = Math.cos(noise * 1.5 * Math.PI / 180 - (timeStop * 0.1)) * 255;
        b = Math.sin(r * 0.15 * Math.PI / 180 + (timeStop * 0.5)) * 255;
        g = 255 - ~~(r * j * i) / 255;
        op = 255;
        break;
      case 'octal':
      ...
      case 'offset':
        ...
        g = Math.sin(i *  Math.PI / grid + (timeStop * 0.1)) * 255;
        r = Math.cos(j * Math.PI / grid + (timeStop * 0.09)) * 255;
        op = 255;
        break;
      case 'rainbow':
        ...
        b = Math.cos(noise * 5 * Math.PI / 180 - (timeStop * 0.3)) * 255;
        op = r > 100 ? 255 : r; // ~~(((noise * 255) - 0) / (255 - 0)) / 1.25;
  			break;
      case 'hashing':
        // original render color mode
        var avg = Math.cos(noise * 2.5 * Math.PI / 180 - (timeStop * 0.1)) * 255;
        r = 255;
        b = r;
        g = b;
        op = avg > 100 ? 255 : 0;
